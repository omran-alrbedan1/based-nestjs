...
In this repository, always prefer senior-level, big-project practices by default.

That means you should bias toward:

- scalable patterns over quick patches
- reusable abstractions over repeated logic
- centralized policies over one-off handling
- maintainability over shortest-possible implementation
- production-safe defaults over demo-style shortcuts

If a simple implementation and a scalable implementation are both possible, prefer the scalable one unless the developer explicitly asks for a lightweight temporary solution.
